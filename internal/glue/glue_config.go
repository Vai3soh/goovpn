...
func (c *Config) RemoveCertsAndKeys() {
	c.Body = regexp.MustCompile(`(?m)^deadline|^ca.+|^key.+|^cert.+|^tls-auth.+|^tls-crypt.+`).ReplaceAllString(c.Body, "")
}
...
func (c *Config) CheckConfigUseFiles() bool {
	return regexp.MustCompile(`(?m)^cert\s+|^ca\s+|^key\s+|^tls-auth\s+|^tls-crypt\s+`).MatchString(c.Body)
}
...
func (c *Config) AddStringToConfig(inFile *os.File) string {
	...
	for scanner.Scan() {
		if !contains(strs, substr) {
			if regexp.MustCompile(`(?m)^ca|^key|^cert|^tls-auth|^tls-crypt`).MatchString(scanner.Text()) {
